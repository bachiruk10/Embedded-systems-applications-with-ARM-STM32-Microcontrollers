...
def animate(i, data1, data2, data3, ser):
    ...
    s_bytes = ser.readline(2)
    #time.sleep(0.05)
    if (len(s_bytes) == 2):
        counter = int.from_bytes(s_bytes, "little", signed= False)
        #t.append(counter)                               # Add to the list holding the fixed number of points to animate
    else:
        try:
            s_bytes += ser.readline(2-len(s_bytes))
            counter = int.from_bytes(s_bytes, "little", signed= False)
            #t.append(counter)
        except:
            print ("missing bytes")
    ...
    data1 = data1[-50:]                           # Fix the list size so that the animation plot 'window' is x number of points
    data2 = data2[-50:]                           # Fix the list size so that the animation plot 'window' is x number of points
    data3 = data3[-50:]                           # Fix the list size so that the animation plot 'window' is x number of points
    ...
    ax1.set_ylim([0, 4]) 
    ax1.set_ylabel("VPOT1 (V)")                              # Set title of y axis 

                                                        # Clear last data frame
    ax2.plot(data2)
    ax2.set_ylim([0, 4])          
    ax2.set_ylabel("VPOT2 (V)")

                                                      # Clear last data frame
    ax3.plot(data3)  
    ax3.set_ylim([0, 4])        
    ax3.set_ylabel("VPOT3 (V)")

    #ax1.set_ylim([0, 4])                                  # Set Y axis limit of plot
    ax1.set_title("ADC STM32 Data")                        # Set title of figure
